...
`configure_aws_target.sh` resolves the account with AWS STS and the region from
`AWS_REGION`, `AWS_DEFAULT_REGION`, or the active AWS profile. Its generated
`agentcore/aws-targets.json` is local state and is excluded from Git.
